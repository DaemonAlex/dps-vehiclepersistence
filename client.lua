...
local propsRequested = {} -- Track which vehicles we've requested props for

-- Check admin status on load and cache it
CreateThread(function()
    Wait(2000) -- Wait for player to be ready
    if Config.AdminExempt then
        isAdmin = lib.callback.await('dps-vehiclepersistence:isAdmin', false)
        if isAdmin and Config.Debug then
            print('[dps-vehiclepersistence] Admin detected - persistence tracking disabled for this player')
        end
    end
end)

-- Recheck admin status when player data changes (job change, etc.)
...
CreateThread(function()
    ...
    if Config.Enabled == false then
        if Config.Debug then
            print('[dps-vehiclepersistence] Persistence DISABLED - skipping vehicle tracking')
        end
        return
    end
    ...
    while true do
        ...
        if isAdmin then
            Wait(5000) -- Slow poll when admin
            goto continue
        end
